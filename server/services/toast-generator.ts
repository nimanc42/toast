...
export async function generateWeeklyToast(userId: number, userName: string = '', forceGenerate: boolean = false): Promise<Toast> {
  ...
  await sendEmailNotificationIfEnabled(userId, userInfo?.name || userName || '', userInfo?.email || '');
 ...
 * Send email notification if user has email notifications enabled
 */
async function sendEmailNotificationIfEnabled(userId: number, userName: string, userEmail: string) {
  try {
    // Skip for test users
    if (userId === CONFIG.TEST_USER.id) {
      return;
    }

    // Get user's email notification preference
    const voicePrefs = await db.execute(sql`
      SELECT email_notifications, user_id 
      FROM voice_preferences 
      WHERE user_id = ${userId}
    `);

    if (voicePrefs.rows.length > 0 && voicePrefs.rows[0].email_notifications) {
    ...
    console.error('[Toast Generator] Error sending email notification:', error);
    // Don't throw error - email failure shouldn't break toast generation
  }
}
